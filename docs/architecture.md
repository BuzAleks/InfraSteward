...
Global scripts are stored once, while workspaces store tagged attachments to those scripts. Multiple attachments can point to the same global script; each attachment has its own tag, parameter overrides, and MCP flag. Workspace-level parameter settings are synchronized from currently attached scripts.
...
The backend is a Tauri Rust crate in `src-tauri/`. It exposes commands for app-data loading/saving, SSH connection saving, connection testing, runtime path discovery, working-directory opening, MCP bridge control, and script execution. Non-secret data is stored as JSON in the configured working data directory. Writes use a temporary file followed by rename.
...
Workspaces can target either SSH or the local machine. SSH execution uses the `ssh2` crate and can prefix execution with a remote `cd` when the connection has a working directory. Local execution spawns a configured shell runner (`bash`, `sh`, `zsh`, Git Bash, WSL bash, or custom command), writes the script to stdin, streams stdout/stderr from child pipes, and terminates the process tree on cancellation or timeout. Manual parameter values are passed as environment assignments, avoiding text replacement inside script content.
